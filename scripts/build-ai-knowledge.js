...
const RELATED_COMMUNITIES = [
  {
    id: "community-tachikawa-any",
    type: "community",
    name: "Tachikawa.any",
    aliases: ["tachikawa.any", "立川.any", "tachikawaany"],
    area: "立川・国立・国分寺・八王子・東大和など",
    description:
      "中央線・多摩モノレール沿線に住む、または働くテックな人たちが気軽に集まれるコミュニティです。",
    connpassUrl: "https://tachikawaany.connpass.com/",
  },
];
...
function extractTalkCards(html, event) {
  ...
  return cards.flatMap((card, index) => {
    ...
    const speakerBlock =
      card.match(/<div class="speaker">([\s\S]*?)<\/div>/i)?.[1] || "";
    const speaker = firstMatch(speakerBlock, /<a[\s\S]*?>([\s\S]*?)<\/a>/i);
    const speakerUrl = speakerBlock.match(/href="([^"]+)"/i)?.[1] || "";
    const badge = firstMatch(
      card,
      /<span class="talkBadge">([\s\S]*?)<\/span>/i
    );
    const speakerDeckUrl =
      card.match(/src="(https:\/\/speakerdeck\.com\/player\/[^"]+)"/i)?.[1] ||
      "";
    const docswellUrl =
      card.match(/<div class="docswell-link">[\s\S]*?href="([^"]+)"/i)?.[1] ||
      "";
    const directMaterialUrl =
      card.match(/class="talkLink"[^>]*href="([^"]+)"/i)?.[1] || "";
    ...
    return [
      {
        ...
        materialStatus:
          speakerDeckUrl || docswellUrl || directMaterialUrl
            ? "published"
            : unavailable
            ? "unavailable"
            : "unknown",
...
function extractEvent(directory) {
  ...
  const name =
    firstMatch(html, /<h1>([\s\S]*?)<\/h1>/i) || `多摩.dev ${directory}`;
  const startDate =
  ...
  const connpassUrl =
    html.match(
      /href="(https:\/\/tamadev\.connpass\.com\/event\/[^"/]+\/?)"/i
    )?.[1] || "";
...
const knowledge = {
  ...
  community: {
    name: "多摩.dev",
    description:
      "多摩地域のエンジニアが、気軽に集まり、学び、つながるコミュニティです。初参加、登壇なし、多摩地域外からの参加も歓迎しています。",
...
console.log(
  `Wrote ${knowledge.events.length} events and ${ knowledge.talks.length } talks to ${path.relative(ROOT, OUTPUT_FILE)}`
);
